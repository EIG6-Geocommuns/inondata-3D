...
export default function setupControls(view: itowns.GlobeView) {
    if (!view.controls) return;

    // controls specific setting.
    // view.controls.dampingMoveFactor = 0.9;
    // view.controls.minDistanceCollision = 0.1;
    // view.controls.minDistance = 20;

    // view.controls.states.setFromOptions(defaultOptions);
    view.controls.states.setFromOptions(demoLidarOptions);
}
